...
```{r bdomain}
barrier1 <- cbind(
  c(0.25, 0.65, 0.63, 0.23, 0.25) * rxy[1], 
  c(0.45, 0.80, 0.85, 0.50, 0.45) * rxy[2])
barrier2 <- cbind(
  c(0.50, 1.50, 1.50, 0.50, 0.50) * rxy[1],
  c(0.35, 0.35, 0.40, 0.40, 0.35) * rxy[2])
```
...
```{r lcorrels}
locs <- cbind(c(0.3, 0.7, 0.7, 0.3) * rxy[1], 
              c(0.7, 0.7, 0.3, 0.3) * rxy[2])
...
Let us build a projector to project any vector from the mesh nodes to 
a fine grid and project the computed correlations with this.
```{r pgrid}
...
we will set the small correlation values as NA so that 
these values will not dominate the image later.
```{r gcorrels}
for(i in 1:ncol(gcorrels)) {
  ii <- which(abs(gcorrels[, i]) < 0.1)
  gcorrels[ii, i] <- NA
}
```

We can now create a `data.frame` with the expanded coordinates of the pixels
and join the correlations with 
```{r grid.df}
grid.df <- data.frame(
...
```
```{r ggcorrel}
ggc <- lapply(1:ncol(gcorrels), function(i) {
  ggplot() + 
    geom_raster(
      data = grid.df, 
      aes(x = x, y = y, fill = grid.df[, 2 + i])) + 
    labs(fill="Correlation") +
    geom_point(aes(x = locs[i, 1], y = locs[i, 2])) +
...
The posterior mean and the standard error can be projected into small pixels
