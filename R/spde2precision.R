...
spde2precision <- function(kappa, fem, alpha) {
  if (alpha==1)
    return(fem$c0*kappa^2 + fem$g1)
  if (alpha==2)
    return(fem$c0*kappa^4 + 2*kappa^2 * fem$g1 + fem$g2)
  if ((alpha>1) & (alpha<2)) {
    lambda <- alpha - floor(alpha)
    b <- matrix(c(1, 0, 0, 1, 1, 0, 1, 2, 1), 3, 3) %*%
      solve(matrix(1/(c(4:2, 3:1, 2:0) + lambda), 3,
                   3), 1/(c(4:2) + lambda - alpha))
    return(kappa^4*fem$c0*b[1] +
             kappa^2*fem$g1*b[2] + fem$g2*b[3])
  }
  return(NULL)
}
